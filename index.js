...
/* ÖRNEK GÖREV:
	cumleKur fonksiyonuna aşağıdakileri uygulayın:
		...
		Yukarıdaki fonksiyonu foo("deneme") olarak çağırabiliriz, bu fonksiyon "deneme" stringini döndürecektir.
		 Aynı fonksiyonu foo(); şeklinde de çağırabiliriz. Bu durumda parametre olarak bir şey göndermediğimiz için fonksiyon içindeki degisken değişkenine false değeri atanacaktır. 
...
/* GÖREV 2:
		paragrafOlustur fonksiyonuna aşağıdakileri uygulayın.
			...
			5. Oluşturulan bu dizinin tek sayılı anahtarlarını(index) [1,3,5,7,9] kullanarak ilk 5 elemanı geriçağırım olarak çağırılan cumleKur dizisinin 
			parametreleri olarak aktarılarak çağırılacak ÖRNEK: callback(dizi[1],dizi[2],dizi[3],dizi[5],dizi[7])
			6. Oluşturulan paragraf döndürülecek
	*/
	
function paragrafOlustur(cumleler, cumleKur,cumlelereDonustur){
	let pgraf = cumlelereDonustur(cumleler, " ");
	
	return cumleKur(pgraf[1],pgraf[3],pgraf[5],pgraf[7],pgraf[9]);
...
/* 	GÖREV 4:
		...
		ÖRNEK: Mesaj içinde ":)" sembolü bulunursa mesajı alan kişi bu sembolü "🙂" olarak görecek. Burdan yola çıkarak emojileriDonustur fonksiyonuna 
...
for(let key in emojiler){
	...
	let emoji = emojiler[key];
	string = low.replace(key,emoji)
